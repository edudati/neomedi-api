...
async def create_visit(
    ...
    - **patient_id**: ID do paciente (obrigatório - busca record automaticamente)
    - **professional_id**: ID do profissional (opcional - obtido do JWT se não fornecido)
    - **company_id**: ID da clínica (opcional)
    - Outros campos clínicos opcionais
    """
    try:
        # Buscar record do paciente
        record = await record_repo.get_by_patient_id(request.patient_id)
        if not record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
                detail=f"Prontuário para paciente {request.patient_id} não encontrado"
            )
        
        # Pegar professional_id do JWT se não fornecido
        professional_id = request.professional_id or current_user.get("user_uid")
        if not professional_id:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail="professional_id não encontrado no token"
            )
...
async def get_visits_by_patient(
    ...
    """
    Busca atendimentos de um paciente com paginação
    ...
    try:
        # Buscar record do paciente
        record = await record_repo.get_by_patient_id(patient_id)
        if not record:
            raise HTTPException(
                status_code=status.HTTP_404_NOT_FOUND,
        ...
        # Buscar visits do record
        use_case = GetVisitsByRecordUseCase(visit_repo)
        visits = await use_case.execute(record.id, limit, offset)
